...
namespace koi_farm_demo.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddStaff([FromBody] AddStaffModel model)
        {
            try
            {
                await _userService.AddStaffAsync(model.Email, model.Password, model.FullName, model.PhoneNumber, model.Role);
        ...
        public async Task<IActionResult> GoogleCallback(string code)
        {
            try
            {
                ...
                return Redirect($"http://localhost:5173/LoginSuccess/{token}");
            }
            catch (Exception ex)
            {
